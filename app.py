...
@app.route("/lowCP", methods=['GET', 'POST'])   #低轉換溢價清單
def lowCP():#4
    global recs
    recs = []
    vip_name=session.get('username')
...
@app.route("/chase", methods=['GET', 'POST'])   #以債追股
def chase():
    global recs
    recs = []
    vip_name=session.get('username')
...
@app.route("/qryName", methods=['GET', 'POST'])     #搜尋功能
def qryName():
    global recs
    recs = []
    vip_name=session.get('username')
...
@app.route("/optional", methods=['GET', 'POST'])    #添加自選(按鈕)
def optional():
    global recs
    # recs = []
    vip_name=session.get('username')
...
@app.route("/wishlist", methods=['GET', 'POST'])    #自選清單
def wishlist():
    global recs
    recs = []
    vip_name=session.get('username')
